...
'New:
'Now using .NET 4.8 (Minimum Windows 7 SP1)
'Updated AlphaFS, (Pro)DotNetZip, SevenZipSharp libraries (now using nuget)

'ISSUES TO CHECK:
'syncing when set to never
'2. Performing a 'restore all files' results in a simple copy of the backup folder to the source ie all file versions in the backup are restored to the source.
'IMHO a restore should only restore the current version of files.
...
'Watch description
'Language support
'Support encryption of backup files (you can use a 3rd party volume for your backup folder in the mean time) 
...
'*** From GitHub:
'Password for zip/7z
'limit backups (count) if filesize > x
'limit the size of zip files (ie 1GB)
'max 1 versio  per minute not working
'save to gdrive/onedrive/webdav
'support backup folder in watch folder (.\backup)
'support relative paths in backup now (..\backup)
'crc hash saved somewhere
